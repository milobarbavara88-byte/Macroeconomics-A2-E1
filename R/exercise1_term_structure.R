...
#
# Authors: <NAME SURNAME 1>, <NAME SURNAME 2>, <NAME SURNAME 3>
#
# This single script solves Questions 1-5 of Exercise 1 exactly in the order
# requested by the assignment. Every block is commented with (i) WHAT we do,
# (ii) WHY we do it and (iii) HOW to read the resulting output / graph.
#
# Two econometric approaches are implemented:
#   - a TWO-STEP dynamic Nelson-Siegel model: the yield curve of each month is
#     summarised by three factors (level, slope, curvature) estimated by OLS,
#     and these factors are then forecast with a VAR (Questions 2-3);
#   - a ONE-STEP state-space dynamic Nelson-Siegel model, where the same three
#     factors are latent states estimated by the Kalman filter (Question 4).
# Question 5 compares and interprets the results.
#
# The code is fully replicable: it downloads the data directly from FRED and
# saves every figure into the ./output folder.
# ==============================================================================


# ------------------------------------------------------------------------------
# 0. SETUP
# WHAT: load (and, if needed, install) the few packages we rely on.
# WHY : 'quantmod' downloads FRED data; 'xts'/'zoo' handle time series; 'vars'
#       estimates the VAR. The Kalman filter is coded by hand (Question 4) so
#       that every step is transparent and no black-box package is needed.
# ------------------------------------------------------------------------------
ensure_pkg <- function(pkg) {
  # Install a package only if it is missing, then load it quietly.
...
set.seed(123)                                   # reproducibility (optim is deterministic, but kept for safety)
dir.create("output", showWarnings = FALSE)      # all figures are written here
...
# WHAT: download daily US Treasury constant-maturity yields from FRED.
# WHY : these are the standard, freely available zero-coupon-like yields used in
#       the Diebold-Li literature. We pick eight maturities that are continuously
#       available and that include the three maturities (3, 24, 120 months)
#       required by the empirical level/slope/curvature formulas below.
#       We deliberately stop at the 10-year (120 months) maturity because the
#       20y/30y FRED series have long gaps that would destroy a balanced panel.
...
# WHAT: merge the eight daily series into one xts matrix, columns = maturities.
daily <- do.call(merge, lapply(fred_ids, get))
colnames(daily) <- as.character(maturities)

# WHAT: convert daily yields to MONTHLY AVERAGES.
# WHY : monthly frequency is the standard frequency for yield-curve forecasting;
#       averaging over the month smooths daily noise and non-trading days.
monthly <- apply.monthly(daily, function(x) colMeans(x, na.rm = TRUE))

# WHAT: keep a clean balanced sample by removing months with any missing maturity.
# WHY : OLS / VAR / Kalman all require a complete cross-section every month.
yields <- na.omit(monthly)
index(yields) <- as.Date(as.yearmon(index(yields)))   # stamp each row at the 1st of its month

# WHAT: report the basic sample information requested by the assignment.
# HOW TO READ: these four facts define the panel we analyse for the whole exercise.
...
# WHAT: plot yields against maturity for one early, one middle and one recent month.
# WHY : a snapshot of the cross-section shows how the SHAPE of the curve changes
#       over time (upward sloping, flat, or inverted).
# HOW TO READ: an upward-sloping curve signals normal expansion expectations; a
#       flat or inverted curve (long < short) often precedes recessions.
